#!/usr/bin/env python3
"""
Build the branded HTML for the Phase 3 liquidity audit from its markdown.

    python build_liq.py

Copied from the Phase 5 build (5-simulations/_branded_working/build_sim.py),
itself generalised from the market-research build. Changes: the brand path is
resolved on the machine running it, the audit is the only document, and the
appendix Part is found by its title rather than its number. Self-contained output: brand CSS,
...
PHASE = os.path.dirname(HERE)                       # deliverables/3-supply-side-tokenomics
...
DOCS = {
    "results": dict(md=os.path.join(PHASE, "Aurumix_Liquidity_Audit.md"),
                    meta=os.path.join(HERE, "meta_liquidity.json"),
                    out=os.path.join(HERE, "Aurumix_Liquidity_Audit.html"),
                    dividers_per="part"),
}
