...
def render_table(header, rows):
    ...
    out = []
    chunks = [rows[i:i + TABLE_CHUNK] for i in range(0, len(rows), TABLE_CHUNK)] or [[]]
    for chunk in chunks:
...
def page_content(heading, body_html):
    head = ""
    if heading:
        head = '<h2 class="content-heading">%s</h2>\n    <hr class="heading-rule">\n    ' % esc(heading)
    return """<div class="page page--content-single-column" data-template="content-single-column" data-auto-continue>
  <div class="content-area">
    ...
    for sec in sections:
        ...
        if num in profile_sections:
            ...
            for g in groups:
                ...
                pages.append(page_content(None, "".join(md_blocks(g))))
        else:
            for ln in body_lines:
                hm = re.match(r"^###\s+(?!#)(.*)$", ln)
                if hm:
            ...
            pages.append(page_content("%s. %s" % (num, title), "".join(md_blocks(body_lines))))
