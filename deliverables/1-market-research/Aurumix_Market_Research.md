...
2. **There is a proven compliant path: deliver the reward off-token, at the account level — and fund it from real yield, not recycled fees.** Kinesis pays a fee-share "holder's yield" to *accounts*; Ondo separates the ownership token from the yield instrument; and the Dubai peer **ORO** funds gold yield from an **external gold-leasing partner (Monetary Metals)** — genuine income framed as yield-on-asset, not a company profit-share. Reward survives; token stays a commodity. 🟡 → **B1, B5**
...
>
> **The ORO / Monetary Metals model — the real-yield answer to Q11.** The Dubai peer ORO (see A7) makes gold income-bearing by plugging into **Monetary Metals, an external gold-leasing business that pays interest on gold, in gold** — genuine outside income, framed as **yield *on the asset*** (commodity-consistent), not a profit-share of the company (security-like). This is a concrete strategic option for Aurumix: **fund the "dividend" from a real external gold-yield source rather than recycling its own fees** — which simultaneously (a) resolves the "recycled fees" critique (Q11), and (b) lowers the securities-classification risk by making the reward asset-yield, not enterprise-profit. ⚠ **Trade-off:** leasing gold out **encumbers it**, denting the "100% allocated / unencumbered" promise — so any such yield must be capped, disclosed, and risk-managed (Monetary Metals manages exactly this counterparty risk). Worth putting on the table for B5 as an alternative/complement to the fee-funded dividend.
...
**⭐ ORO / ORO Gold (Dubai) — the closest comparable found.** ORO Labs (Dubai, founded 2024; founders Usman Saleem, Juan Marchetto) runs a **yield-bearing tokenized gold protocol** — `$GOLD` SPL token on Solana + the ORO app on Fasset's Ethereum L2 "Own" (first app on Own, Dec 2024). **1 token = 1 troy oz, 99.99% fine, LBMA + UAE GD-certified**, insured vaults, **Brinks** custody, a **bankruptcy-remote foundation** holding gold for token holders, **monthly RSM audits**, mint-only-after-gold-acquired. **Zero storage fee** to holders. **Physical redemption from ~$85 increments OR USDC, after a 1-day lock-in** (fractional — unlike PAXG's full-bar minimum). ✅ Launching through 2025 (invite-only beta → public Q3 2025); stated goal to tokenize **$1B** of yield-bearing gold in year one. Exact UAE regulator (VARA vs DMCC) not publicly stated — 🔴 unverified. **The critical detail:** its **yield comes from an external source — a partnership with Monetary Metals (May 2025), a real gold-leasing business that pays interest on gold, in gold** — not from recycling platform fees. ORO overlaps Aurumix on gold + yield + Dubai + fractional redemption + borrowing more than any other product; it does **not** do SIP accumulation, ICS/tier gamification, mining oversubscription, or inheritance. 🟡 (some specifics single-source/press-based)
...
| 16 | ORO (Dubai): real yield via external gold-leasing (Monetary Metals) — the compliant, non-recycled route for the dividend | 🟡 | B5, B1 |
