...
(async () => {
  ...
  const report = await page.evaluate(() => {
    ...
    // Sparse pages: not literally empty, so the check above misses them, but
    // visually they are a widow. A trailing paragraph or callout pushed onto
    // its own page leaves 80%+ of the sheet blank and reads as a defect.
    const sparse = [];
    pages.forEach((p, i) => {
      const ca = p.querySelector('.content-area');
      if (!ca) return;
      const txt = (ca.textContent || '').trim();
      if (!txt.length) return;                       // already counted as empty
      ...
      const fill = (lowest - areaTop) / areaH;
      if (fill < 0.35) {
        sparse.push({
          index: i + 1,
          printed: (p.querySelector('.page-number') || {}).textContent || '(unnumbered)',
    ...
    return { total: pages.length, numbered, overflow, empties, sparse, tocEntries: toc.length, unresolved, toc };
  });
