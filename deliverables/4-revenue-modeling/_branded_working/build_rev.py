#!/usr/bin/env python3
"""
Build the branded HTML for the Phase 4 Revenue Model Explainer from its markdown.

    python build_rev.py

Copied from the Phase 3 build (3-supply-side-tokenomics/_branded_working/build_liq.py).
Changes: numbered-section layout (as the Phase 5 setup document) with the
executive summary page in front, and Mermaid diagrams rendered in the page
before the splitter measures it. Self-contained output: brand CSS,
...
PHASE = os.path.dirname(HERE)                       # deliverables/4-revenue-modeling
BRAND = os.path.join(os.path.expanduser("~"), ".claude", "plugins", "marketplaces", "tokenomics-net",
                     "tokenomics-plugin", "standards", "branded-docs-main")

DOCS = {
    "explainer": dict(md=os.path.join(PHASE, "Aurumix_Revenue_Model_Explainer.md"),
                      meta=os.path.join(HERE, "meta_rev.json"),
                      out=os.path.join(HERE, "Aurumix_Revenue_Model_Explainer.html")),
}

LIST_CHUNK = 4    # lists are split into runs of this many items between blocks
...
def md_blocks(lines, ctx):
    ...
    while i < n:
        ...
        # mermaid diagram: rendered in the page, before the splitter runs
        if s.startswith("```mermaid"):
            ...
            while j < n and not lines[j].strip():
                j += 1
            cm = re.match(r"^\*(?:Figure|Diagram)[^*]*?[.:]\s*(.+?)\*$", lines[j].strip()) if j < n else None
            if cm:
                cap, i = cm.group(1), j + 1
            # a left-to-right chain of five or more boxes is wider than the page and
            # scales its labels down to ~6pt; drawn top-down it keeps full size
            nodes = set(re.findall(r"\b([A-Z])\[", "\n".join(buf)))
            if len(nodes) >= 5:
                buf = [re.sub(r"^(\s*)graph LR\b", r"\1graph TB", x) for x in buf]
            ctx["fig"] += 1
            blocks.append('<figure class="diagram"><div class="mermaid">\n%s\n</div>%s</figure>'
                          % (esc("\n".join(buf)),
                             '<figcaption>Figure %d. %s</figcaption>' % (ctx["fig"], inline(cap)) if cap else ""))
            continue
        ...
        if s.startswith(">"):
            ...
            head = '<div class="callout-title">%s</div>' % inline(title) if title else ""
            blocks.append('<div class="callout">%s<div class="callout-body">%s</div></div>' % (head, inline(text)))
            continue
        ...
        if re.match(r"^[-*+]\s+", s):
            ...
            while i < n:
                ...
                if re.match(r"^[-*+]\s+", t):
                    ...
                    break
            # long lists go out in chunks so the page splitter can break them
            for k in range(0, len(items), LIST_CHUNK):
                blocks.append("<ul>%s</ul>" % "".join("<li>%s</li>" % inline(x) for x in items[k:k + LIST_CHUNK]))
            continue
        ...
        if re.match(r"^\d+\.\s+", s):
            ...
            for k in range(0, len(items), LIST_CHUNK):
                first = int(start) + k
                attr = ' start="%d"' % first if first != 1 else ""
                blocks.append("<ol%s>%s</ol>" % (attr, "".join("<li>%s</li>" % inline(x) for x in items[k:k + LIST_CHUNK])))
            continue
...
(function(){
  ...
  function start(){
    // formulas and diagrams must be drawn before pages are measured and split
    var k=window.__katexReady||Promise.resolve();
    k.then(function(){return document.fonts.ready;}).then(function(){
      // after the web fonts, or mermaid sizes its boxes to the fallback font and clips the labels
      ...
      return window.mermaid.run({querySelector:'.mermaid'}).catch(function(e){console.error(e);});
...
.page--content-single-column .content-area ul + ul,
.page--content-single-column .content-area ol + ol { margin-top: -6px; }
...
/* diagrams */
.content-area figure.diagram { margin: 10px 0 14px; break-inside: avoid; text-align: center; }
.content-area figure.diagram svg { max-width: 100%; max-height: 5in; height: auto; }
.content-area figure.diagram figcaption {
...
def build_results(meta, md, ctx):
    ...
    for sec in sections:
        ...
        for g in groups:
            ...
            if t.lower() == "recommendations":
                ...
                pages.append(("recs", ("Recommendations",
                                       "Each recommendation is tied to a finding in this audit.",
                                       cards, "sec-recs")))
                continue
        ...
        if title.lower() != "appendix" and (lead or body_groups):
...
def main():
    which = sys.argv[1] if len(sys.argv) > 1 else "explainer"
    d = DOCS[which]
    ...
    # executive summary page straight after the contents
    pages.insert(2, ("exec", None))
    toc.insert(0, {"level": 1, "text": "Executive summary", "ref": "sec-exec"})
    body_pages = render_pages(pages, toc, meta, ctx, logo_dark, logo_light)

    has_math = "\\[" in "".join(body_pages) or "\\(" in "".join(body_pages)
    has_mermaid = 'class="mermaid"' in "".join(body_pages)
    head_js = (KATEX_HEAD if has_math else "") + (MERMAID_HEAD if has_mermaid else "")
    out = """<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
""" % (esc(meta["title"]), head_js, brand_css, tpl_css, EXTRA_CSS,
       "\n".join(body_pages), AUTO_CONTINUE)

    with open(d["out"], "w", encoding="utf-8") as f:
        f.write(out)
    print("wrote %s" % d["out"])
    print("source pages before auto-continue: %d | toc entries: %d | figures: %d | math: %s | mermaid: %s"
          % (len(pages), len(toc), ctx["fig"], has_math, has_mermaid))
