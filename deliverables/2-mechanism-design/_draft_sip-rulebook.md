...
3. **A period counts** if the month's contributions total at least the product floor and the money cleared inside the month or its grace window.
...
| **Counted period** | A month in which cleared contributions **totalled at least the floor**, within the month or its grace window | Tested on the monthly total, so splitting one payment into three is harmless and topping up an under-payment works automatically |
| **The floor** | A single product-wide minimum: **USD 20** or the local equivalent | See §3. This replaces the per-customer "declared minimum" |
| **Declared amount** | What Aurumix asks the bank for each month. Variable | The collection instruction. It is not a promise and it is never scored |
| **Mandate ceiling** | The maximum the mandate authorises per month | The declared amount moves freely beneath it. Only raising the ceiling needs a new mandate |
...
**This also answers "what happens on a payment below the minimum", which `handoff.md` §7 records as undefined.** The question dissolves. A payment below USD 20 is not a smaller contribution, it is an incomplete one, and the investor has the rest of the grace window to top it up to USD 20. The gold from the partial payment is allocated immediately either way, because the gram count only rises.

> **Rejected: treating an under-payment as a declared pause.** A pause freezes rather than steps down *because it is a communication*, and communication is the behaviour the score exists to reward. An under-payment communicates nothing. The top-up window is the correct concession and it costs the honest customer nothing.
...
### 6.2 The mandate carries a ceiling, not an amount

| Rail | Investor action | On failure | Use |
|---|---|---|---|
| **UAEDDS direct debit** | None | **The investor's own bank charges them AED 105** | Default |
| **AANI Request to Pay** | One tap | Nothing. No fee | Alternative, and mandatory after two failures |
| **AANI electronic direct debit** | None | Unknown | Target state. Announced, not live |

**Rules for the mandate:**

- The mandate authorises **up to a ceiling** per month. The declared amount moves freely beneath it with no new mandate. Raising the ceiling needs a new mandate.
...
- If the declared amount exceeds the ceiling, the collection **fails** rather than under-collects, and it is treated as a failure (§7), not as non-payment.
- **Never auto re-present a failed UAEDDS debit. Not once.** Every re-presentment costs the investor about USD 28.59. Notify, tell them their bank has charged them, switch that period to a push request inside grace, and move them to push permanently after two failures.

> ⚠ **One unverified dependency, and it is load-bearing.** Whether UAEDDS supports **variable-amount mandates under a ceiling** is not confirmed (`handoff.md` §7, item 11). **If it does not**, the fallback is a fixed-amount mandate at a conservative figure plus push top-ups, which still satisfies the floor test because counting runs on the monthly total. The design survives either way, but the marketing does not: do not promise "set it and forget it" on the push rail. Say **"one tap a month"** and mean it.
...
|---|---|---|---|
| Month's total at or above the floor, on time | Allocated | **Accrues** | None |
| Month's total at or above the floor, paid **late but inside 15-day grace** | Allocated at the fix on the day funds clear | **Unaffected** | Push nudges on days 3, 8, 14 |
| **Partial: paid something, below the floor** | **Allocated immediately** | Unaffected during grace. Counts if topped up to the floor inside grace | Push request for the shortfall |
| **Debit rejected** | None yet | Unaffected during grace | **No auto re-presentment.** Switch to push |
| **Nothing cleared by the end of grace** | **Untouched** | **Continuity halves. Tenure, Retention and gold untouched** | Mandate retained |
| **Arrears cleared inside 12 months** | Allocated at the fix on the day arrears clear | **Fully restored**, as though the period had been paid | Normal |
| Spot purchase | Allocated | **No period** | n/a |

### 7.2 The rules underneath the table

- **Grace is 15 days** from the contribution date, the IRDAI standard for monthly-mode premiums.
- **Grace crosses the month boundary and settles the period it was due.** A payment on 5 March for a 28 February date closes February. **March still needs its own payment.** A late payment can never close two periods.
...
**Record the two separately in the data from day one**, even though they run the same ladder. Without the distinction there is no way to know whether the ladder is calibrated correctly or whether it is quietly executing customers for a bank timing problem.
...
|---|---|---|---|---|---|---|
| **Declared pause** | The investor tells you. Up to **3 periods, once in 12 months** | Retained | **Frozen** | Frozen | Keeps running normally | Retained |
| **Regulatory pause** | They move somewhere Aurumix cannot serve | Retained | **Frozen** | Frozen | Keeps running normally | Retained |
| **Stop** | The investor cancels | Retained | Decays as normal | Held | Keeps running normally | **Retained** |
| **Dormant** | 12 consecutive silent periods | Retained. Account hold-only | Held at its decayed level | Held | Keeps running normally | **Retained** |

**Why a declared pause freezes rather than steps down:** a declared pause is a communication, and communication is exactly the behaviour the score exists to reward. An investor who tells you they are pausing is worth more than one who silently misses, and the design should price that difference. Abuse is bounded by the cap.

**Why a regulatory pause freezes:** a forced stop is not a broken promise and must never be scored as one. Same principle as the grace period, applied to a legal event rather than an administrative one. Existing credit runs to term; no new draws.

**Retention needs no special rule in any of these states**, which is a useful property of measuring holding rather than paying. A paused investor who holds their gold keeps R = 1.00 automatically.
...
| 5 | Split one payment into five small ones | Five periods | The floor is tested on the **monthly total** |
...
- [ ] Client's document §6.3: "six consecutive events at the same monthly commitment" resolves to **six consecutive calendar months in which cleared contributions totalled at least the floor.**
...
|---|---|---|
| **Does UAEDDS support variable-amount mandates under a ceiling?** | Us. Contained check, do it first | If not, the amount-variable decision forces everyone onto the push rail and reintroduces the monthly decision the SIP exists to remove |
| The **weights and thresholds** in §11 | Us, B4 | Proposed, not settled. The structure holds whatever the numbers become |
| **How the discount ladder is funded**: uplift to the base rate, or out of margin | **Client.** Their revenue | §1.1. At Year 1 margin is 0.85pp, so a margin-funded ladder is about 0.12pp per tier and the lever is decorative. **Settle it once for the tenure rebate too** |
...
| 15-day grace, monthly mode | IRDAI Master Circular, clause 8(a) | **High.** Verified |
...
| ENBD returned direct debit fee AED 105 | ENBD Schedule of Charges | **Medium-High** |
| Direct debit failure 2-3%, recurring card 10-15% | Recurly, GoCardless | **Medium** |
| UAEDDS variable-amount mandates under a ceiling | **Unverified** | **None.** See §13 |
