...
> 🔄 **REVISED 2026-08-13 (Abdur), five changes, and two of them make the design more principled than the version they replace.** **(1) The pool is 30% of the entry fee**, explicitly a placeholder locking against the revenue model. **(2) The USD 75 counting cap is removed** and the reward is computed on whatever the referee actually contributed, which makes it a **constant 19.4% of lifetime value at every ticket size** instead of collapsing to 3% for the best customers. **(3) The reward is credited in grams from launch**, which moves the gram-credit rail into the September build. **(4) There is no cap on how many people a member may refer**, which is what VARA's own referral case study describes, and single level now carries the anti-MLM defence alone. **(5) The agent network is a separate population**, not a rung above this one, so the automatic graduation door is deleted. ⚠ **Removing the counting cap opened one hole and it is CONSCIOUSLY ACCEPTED**, register item 8: inflate the referee's contributions for exactly six months, then collapse to the floor, buying gold at an effective 3.5% fee instead of 5%. A review mechanism was proposed and **rejected**. **The exposure is 0.65% of whatever goes through the window, with no ceiling**, and it is accepted because the gamer is still down 3.5%, a real customer arrives either way, and anything large enough to matter engages source-of-funds first. §4.4 carries the acceptance and the trigger that would reopen it.

> ⚠ **This draft has been through an adversarial review and it did not survive intact.** Three independent passes ran against the first version: an arithmetic check, a consistency check against the prior record, and a red team. **They found one false invariant that four separate conclusions were resting on, one rule missing without which decision 46 reopened by the back door, one trigger that could never fire for a returning customer, and a privacy disclosure that would have gone into a VARA licence application.** All are fixed and each fix is marked in place with what it replaced, so the reasoning is auditable rather than tidy.

> ⚠ **One review finding was itself wrong, and it is left visible as a caution.** The red team reported a prefund-then-refund exploit as the only genuinely cash-extractive hole in the design, and a thirty-day settling period was built to close it. **On re-examination the attack does not complete** (§4): once six contributions have cleared there is no undrawn balance left to refund, and the launch rails are push-only. **The settling period is removed and register item 14 is reclassified as structural.** An adversarial reviewer under instruction to find flaws will sometimes find one that is not there, and a defence built on an unexamined finding costs the honest customer real time.
...
Formally: the reward pool is **30% of the entry fee the referee actually paid over the six contributions of their qualifying run**, on whatever they contributed with **no counting cap**, split equally between referrer and referee, and **credited in grams to both accounts when the gate resolves.**
...
> 🔄 **A thirty-day settling period was proposed and is REMOVED (Abdur, 2026-08-13). The reward pays the day the gate resolves.**
>
> It had been introduced to close what the adversarial review called the only genuinely cash-extractive hole in the design: **prefund six months, let it draw down, pass the gate, collect both halves, then request the prefunded balance back.** On re-examination that attack does not survive contact with the product.
>
> **By the time the gate resolves there is no prefunded balance left to refund.** The refundable thing under rulebook §6.3 is the **undrawn** balance sitting in the client money account. Each month's drawdown buys gold and transfers title, so once six contributions have cleared, all of it has become the customer's metal. The only way out of metal is the **buyback**, which is an exit at the prevailing fix, not a refund, and the entry fee is not returned on it. **If an undrawn balance still exists, fewer than six contributions have cleared and the gate has not opened.**
>
> **And the launch rails do not reverse.** AANI Request to Pay and prefunded drawdown are **push** payments, and a completed push is effectively irrevocable. There is no card chargeback exposure because card was ruled out for SIP collection on cost grounds (decision 28).
>
> **So the delay was buying nothing and costing the honest customer a month.** Removed.

**What survives is a standing rule, not a waiting period:**

> **If a contribution forming part of a qualifying run is ever reversed, the reward is void and recoverable.**

One line in the terms, no delay for anyone, and no machinery to build today because nothing on the launch rails can trigger it. ⚠ **It becomes live the day a pull rail ships.** AANI electronic direct debit is adopted when it goes live (decision 43), and a pull carries reversal rights that a push does not. **Revisit this rule at that point, not before.**
...
> ✅ **CONSCIOUSLY ACCEPTED, NOT DEFENDED (Abdur, 2026-08-13).** A contribution-collapse review was proposed and **rejected**. No mechanism is built and no reward is held back on this pattern.

**The exposure, stated so the acceptance is an informed one.** The loss on a gamed referral is the reward minus the margin retained on the run, and both are fractions of the same number:

> **Loss = 1.50% − 0.85% = 0.65% of whatever the referee put through during the six qualifying months.**
>
> USD 12,000 through the window costs USD 78. USD 120,000 would cost USD 780. **There is no ceiling on it**, because there is no counting cap.

**Why it is acceptable anyway, in three parts:**

1. **The gamer is still down.** They pay a 5% fee to recover 1.5%. Nobody makes money doing this; they buy gold at 3.5% instead of 5%. **Invariant 2 (§5.2) holds and is what makes this a discount rather than an extraction.**
2. **A real customer arrives either way.** They are KYC'd, funded from their own account, scored, and holding metal. Aurumix collected the fee, keeps the assets under management, and earns on every downstream stream. **The worst case is a discounted customer, never a fictitious one.**
3. **The persona bounds it in practice.** A USD 2,000/month SIP is already far outside the USD 20 to 75 target, and anything large enough to matter engages source-of-funds and AML review on its own account, before this rule would ever have fired.

⚠ **What would reopen it:** evidence that front-loaded six-month runs are a material share of new volume, which the attribution data will show. **The lever if it does reopen is a counting cap set well above the target (three times, not the USD 75 that was removed), not a discretionary review** — a cap is arithmetic, a review is a judgement, and this design prefers arithmetic wherever it can get it.
...
Meera's score starts at Silver 25 the same day, her entry-fee discount switches on, and the gold lands in both accounts. **One event, everything at once.** Neither party gets a score point, a badge, a tier, or a status.
...
### 6.2 What replaces the cap: one reserved right, no automatic cutoff

**One, not two.** A contribution-collapse review was proposed and **rejected** (§4.4); that pattern is accepted and priced rather than policed. What remains is the same instrument as the round-trip flag (rulebook §9.3: a reserved right, exercised on review, with a stated appeal path).

> **The volume review.** Aurumix reserves the right, where one identity is introducing at a volume that is plainly a distribution business rather than a personal network, to require that the relationship be put on a proper footing before further rewards are paid. **No threshold is published**, because a published threshold is a target and because the honest trigger is a pattern rather than a number.

⚠ **Note what this right is not for.** It exists for the §7.1 risk, an unregistered person building a distribution business. **It is not a backdoor for the collapse pattern**, which is a decided acceptance, and it must not be used as one.
...
| **Must not scale with capital** | **Passes.** The reward does not move with the referrer's holdings at all. The referee's side scales with what the referee paid in fees on their own contributions (no counting cap, §4.4), which is a commission on their purchases, not a return on anyone's holdings |
...
| 22 | **A contribution in the run is reversed after the gate** | **The reward is void and recoverable.** A standing rule, not a waiting period | 🔄 The thirty-day settling period that once carried this is **removed** (§4). It was buying nothing: once six contributions have cleared there is no undrawn balance left to refund, and the launch rails are push-only so a cleared contribution does not reverse. **The rule becomes live machinery only when a pull rail ships** |
...
**So the position is: the loss is real, it is bounded, it is not worth a mechanism, and it is disclosed rather than defended.** The member programme therefore has **no holdback, no vesting, no settling period and no forfeiture window.** The reward pays the day the gate resolves. The only survivor is a standing terms rule that a reversed contribution voids the reward (§4, edge case 22), which builds nothing today.
...
| 6 | **Referring cyclers** who pay six months then withdraw | Rewards on customers worth nothing | **Priced, not prevented.** The loss is **0.65% of what the referee put through the window** (reward 1.50% less margin 0.85%), so about USD 2.93 on a target-ticket run. Judged not worth a mechanism, and disclosed rather than defended (§10.2) | **Accepted** |
| 7 | **Recruiting a downline** | Second-tier earnings | **They do not exist** (§7). There is nothing to build | **Structural** |
| 8 | **Inflate the referee's contributions for exactly the six qualifying months, then collapse to the floor** | Buying gold at an effective 3.5% entry fee instead of 5%. **USD 90 a side on a USD 2,000/month run, against USD 102 of margin, then USD 0.17 a month** | ✅ **CONSCIOUSLY ACCEPTED, not defended (Abdur, 2026-08-13).** Same row as item 6 and the same arithmetic: **0.65% of whatever went through the window, with no ceiling.** The gamer is still down 3.5%, a real KYC'd customer arrives either way, and anything large enough to matter engages source-of-funds first. §4.4 carries the acceptance and the reopening trigger | **Accepted, uncapped** |
...
| 14 | **Prefund six months, pass the gate, collect, then request the refund** | The whole pool, with the money returned | 🔄 **The attack does not complete.** By the time the gate resolves every contribution has cleared into metal, so **there is no undrawn balance left to refund**; an undrawn balance means the gate has not opened. The exit is a buyback at the fix, not a refund, and it returns no entry fee. Push rails do not reverse. **Reviewed and found not to be a hole; the thirty-day window built for it is removed** | **Structural** |
...
| 3c | ✅ **The front-loading pattern is accepted, not defended** (§4.4, register item 8). **Not an open item.** What is owed is monitoring only: report front-loaded six-month runs as a share of new volume once attribution data exists | Us, from month 13 | Nothing. The reopening lever if it is ever needed is a counting cap at three times the target, not a review |
