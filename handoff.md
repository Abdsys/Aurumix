...
All live in `deliverables/2-mechanism-design/`: allocation and float in `_draft_allocation-and-float.md`, SIP/spot/ICS in `_draft_sip-spot-and-ics.md`, entities/licences/ownership/payments in `_draft_entities-licensing-and-payments.md`, and four process-map sets.
...
3. 🔴 **Confirm the India perimeter**, and the **inheritance carve-out** for Digital Will beneficiaries. **Determines the addressable market.**
...
- 🔄 **Phase 2 IN PROGRESS.** Three draft sections, an explainer, and four process-map sets written. Solved: Mining Events, denomination, the dividend, the SIP/spot split, the lock-in, the **structure** of ICS, custody-fee recovery, and 🆕 **the whole legal and operating structure** (ownership model, entities, licences, payment rails, India). **Not yet written: the ICS formula itself and the reconciliation register.**
...
1. 🆕 🔴 **THE TIMING COLLISION, and it goes first because delay has real cost.** Their app is due **early September**; our Data Room lands 4 to 11 September. **The token standard is now load-bearing for the licence, not just for ICS.** If their developers are building a plain transferable ERC-20, that work may have to be thrown away, because an anonymous bearer token cannot evidence who owns the gold and therefore cannot support direct ownership. **Raise it before a rebuild raises it.**
