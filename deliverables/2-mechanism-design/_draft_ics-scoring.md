...
### 1.0 Each component, precisely

- **A counted period** (the atom of the whole score): a calendar month in which one accepted SIP contribution — at or above the USD 20 floor, cleared on time or inside grace — was allocated. One per month maximum, amount irrelevant, spot purchases never count, a missed month is permanently uncounted (§4).
- **Tenure** counts counted periods over the account's life; it only rises. **The cap at 60 is load-bearing, not cosmetic.** Uncapped, tenure alone crosses the Sovereign threshold (84) at about seven years, and from that day the streak is decoration: a veteran could take strike 3 every year and hold the top tier on seniority alone — **uncapped tenure makes the score miss-proof, and behaviour scoring collapses into "survive seven years, then do whatever."** Capped at 60, the split is the one the product wants: **history alone earns a permanent floor (tenure 60 = Titanium); the crown requires history *plus* a live streak** (Sovereign = 60 + at least 12 of continuity). The cap binds the score only — the app keeps displaying the raw month count ("87 months saved"), which keeps rising forever.
- **Continuity** is points, 0–24, one per consecutive on-time month — *not* a 0-to-1 factor. Strikes cut it (§3); it adds to tenure.
- **Supplementaries** are points, 0–13 (§5). Referrals: 1 point per 6 R-weighted referee-months, at most 2 points per referee — four sustained real referees to max. Family: 1 point per sub-account with ≥6 counted periods in its trailing 12 months, at most 5.
- **Retention** is the only 0-to-1 quantity: R = lifetime gram-months held ÷ lifetime gram-months if never sold, starting at 1.00. R_applied = min(1, R/0.80) — keep ≥80% of your lifetime gram-months and the multiplier is exactly 1. ⚠ The allowance is 20% of **time-weighted gram-months, not of grams**: selling early consumes more allowance than selling the same grams late, which is precisely what makes pre-review window-dressing worthless.
- **Why 1 point per month, and not something finer:** the month either had a qualifying payment or it didn't — binary, auditable, unfakeable, and the customer can compute their own score by hand. Points-per-dollar would reintroduce amount (the founding principle forbids it); points-per-day adds noise and timing surface; and "one counted period per calendar month" is the single rule that kills prepayment, payment-splitting and date-shuffling at once (rulebook §9.1 items 4–6). The "1" itself is a normalization — the substance is the 60/24/13 ratio (history 62% / form 25% / engagement 13%), which is the real dial.

### 1.0a Absolute, not relative — a load-bearing choice

Tiers are **absolute thresholds** (score ≥ 84 is Sovereign, for everyone, always), never **relative position** (top 10% is Sovereign). Four reasons:

1. **Relative scoring makes your tier depend on other people's behaviour.** Under percentiles, a flawless saver can be demoted because a better cohort joined — the product's central promise ("never miss for five years on USD 20 and you reach the top") becomes unpromisable, because nobody controls their own outcome. Absolute thresholds are the only structure under which the promise is a contract with yourself.
2. **The benefits are prices and contract terms, so the tier must be quotable.** LTV strikes at draw and runs to term, card levels change at statement cycle, discounts strike per event. A percentile tier flickers with population churn; no lending partner, card sponsor or regulator accepts terms keyed to a moving population statistic.
3. **Relative is zero-sum, and zero-sum is adversarial.** Every new good saver would push an existing saver down — growth punishing loyalty. It also builds a tournament, and a tournament for financial benefits is a compliance surface: **the client's 80/20 dividend was exactly this pathology** (top 10% by score take 80% of the pool), and it is among the reasons the dividend became Gold Rewards. The tier structure must not re-import it.
4. **The one thing rank-ordering buys — automatic rationing of a fixed pool — this design does not need.** Every benefit is self-funding per customer (Gold Rewards capped at that customer's generated revenue, discounts bounded by money handed over, LTV on the partner's book). The funding caps already do the cost control that percentiles exist to do.
...
Two subtleties for the client conversation:

- **Their §8.2 calls continuity the "primary, dominant driver", and in this formula it still is, despite tenure's larger cap.** The behaviour *pay every month* earns both components at once — it drives 84 of the 97 points. The split is not two behaviours weighted differently; it is one behaviour split into its **permanent part** (the month happened — tenure) and its **fragile part** (the run is unbroken — continuity). Continuity is also where all the downside lives (strikes), which is what "dominant driver" means operationally.
- **Retention is the one component deliberately not weighted additively, and that is itself a weighting decision.** As an additive term, holding could be traded off: a cycler with maximal payment points would still score ~84 and take Sovereign. As a multiplier it has **veto power** — no amount of paying, referring or family-building compensates for not keeping the gold. Holding is not one virtue among several; it is a condition on all of them.
