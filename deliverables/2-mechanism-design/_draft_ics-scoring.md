...
- [x] ✅ 🆕 **A scoring map set now exists: `Aurumix_Process_Maps_ICS_Scoring.md`, 10 diagrams (2026-08-13).** Companion to the benefits set, not a replacement: this one explains how the number is earned, that one what it buys, and its map 5 is the bridge carrying the full tier × benefit matrix. Covers the gate, the three facts, the formula, **why minimum and not a sum**, why Retention multiplies, the ladder, the climb, the miss, the cycler, and **a dedicated map on the four departures from the client's §8.2**. Gold Rewards rates are marked **provisional** pending the interchange share and the credit-versus-prepaid decision. Diagram 0 of the benefits set corrected to "six in a row".
