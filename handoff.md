...
- **✅ DONE (2026-07-22):** em dashes (—) removed from `deliverables/1-market-research/Aurumix_Market_Research.md` via a two-pass run of the **`tokenomics:web3-copywriter`** then **`tokenomics:humanizer`** agents (both available as agents this session). ~80 em dashes reworded (colons, sentence splits, parentheses); **0 remain**. En dashes (–) in numeric ranges (2–5%, 6–9 months, 2025–2026) left untouched (16 preserved); all facts, figures, confidence tags, arrows, citations, and tables intact. Pushed to main (commit `6b206ae`).
  - ⚠ Agent-availability note: availability is per-session — in an earlier 2026-07-22 session these agents were deregistered and only the **skills** were available; this session the agents worked. Check the available-agents list each session; use the agents if present, otherwise invoke the skills via the Skill tool.
